...
fn test_governance_deployment_extended() {
    ...
    assert!(token != contract_address_const::<0>(), "Token not deployed");
    assert!(controller != contract_address_const::<0>(), "Controller not deployed");
    assert!(governor != contract_address_const::<0>(), "Governor not deployed");
...
fn test_voting_power_transfer() {
    ...
    assert!(votes.get_votes(VOTER1()) == initial_amount - transfer_amount, "VOTER1 votes not updated");
