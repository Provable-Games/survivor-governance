...
// use openzeppelin_utils::cryptography::nonces::interface::{INoncesDispatcher, INoncesDispatcherTrait};
use snforge_std::{
...
fn test_votes_after_transfer() {
    ...
    assert!(
        votes.get_votes(OWNER()) == INITIAL_SUPPLY - transfer_amount,
        "Wrong OWNER voting power"
    );
