...
fn test_fz_001_balance_consistency(
    transfer_count: u8, amounts: Array<u128>, from_indices: Array<u8>, to_indices: Array<u8>,
) {
    ...
    let transfer_limit = if transfer_count > 50 {
        50
    } else {
        transfer_count
    };
...
fn test_fz_002_delegation_consistency(
    delegation_count: u8, delegator_indices: Array<u8>, delegatee_indices: Array<u8>,
) {
    let token = deploy_token();
    let erc20 = IERC20Dispatcher { contract_address: token };
    ...
    let delegation_limit = if delegation_count > 50 {
        50
    } else {
        delegation_count
    };
...
fn test_fz_003_allowance_safety(
    ...
    let operation_limit = if approval_count > 20 {
        20
    } else {
        approval_count
    };
        ...
        if k < approve_amounts.len() && k < owner_indices.len() && k < spender_indices.len() {
            ...
            if owner != spender && approve_amount > 0 {
                ...
                if k < transfer_amounts.len() {
                    ...
                    if transfer_amount <= approve_amount
                        && transfer_amount <= erc20.balance_of(owner) {
                        ...
                        assert(
                            remaining == approve_amount - transfer_amount,
...
#[test]
#[fuzzer(runs: 50)]
fn test_fz_004_checkpoint_ordering(delegation_changes: Array<u8>, accounts_to_delegate: Array<u8>) {
    let token = deploy_token();
    let erc20 = IERC20Dispatcher { contract_address: token };
    let votes = IVotesDispatcher { contract_address: token };
    ...
    let change_limit = if delegation_changes.len() > 20 {
        20
    } else {
        delegation_changes.len()
    };
    let mut k: u32 = 0;
