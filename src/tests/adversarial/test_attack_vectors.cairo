...
fn VICTIM() -> ContractAddress {
    contract_address_const::<'VICTIM'>()
}

fn WHALE() -> ContractAddress {
    contract_address_const::<'WHALE'>()
}
...
fn test_vote_buying_resistance() {
    ...
    assert!(votes.get_votes(ATTACKER()) == 100000000000000000000000, "Should have votes after delegation");
}
...
fn test_sybil_attack_mitigation() {
    ...
    let sybil1 = contract_address_const::<'SYBIL1'>();
    let sybil2 = contract_address_const::<'SYBIL2'>();
    let sybil3 = contract_address_const::<'SYBIL3'>();
    ...
    let total_sybil_votes = votes.get_votes(sybil1) + votes.get_votes(sybil2) + votes.get_votes(sybil3);
...
fn test_zero_address_delegation_prevention() {
    ...
    start_cheat_caller_address(token, ATTACKER());
    votes.delegate(contract_address_const::<0>());
    stop_cheat_caller_address(token);
    
    // Voting power should be effectively burned
    assert!(votes.get_votes(ATTACKER()) == 0, "Should have no votes");
    assert!(votes.get_votes(contract_address_const::<0>()) == 0, "Zero address has no votes");
}
