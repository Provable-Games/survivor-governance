...
# SurvivorToken Parameters
TOKEN_NAME="${TOKEN_NAME:-Survivor Token}" # Default token name
TOKEN_SYMBOL="${TOKEN_SYMBOL:-SURVIVOR}" # Default token symbol
INITIAL_SUPPLY="${INITIAL_SUPPLY:-1000000000000000000000000}" # 1,000,000 tokens with 18 decimals
# TOKEN_RECIPIENT must be explicitly set - no default
if [ -z "${TOKEN_RECIPIENT:-}" ]; then
    print_error "TOKEN_RECIPIENT must be set in the .env file"
    print_error "Please add: TOKEN_RECIPIENT=0x... (your recipient address)"
    exit 1
fi
...
# These will be set after deploying SurvivorToken and SurvivorGovernorController
...
echo ""
echo "  SurvivorToken Parameters:"
echo "    Token Name: $TOKEN_NAME"
echo "    Token Symbol: $TOKEN_SYMBOL"
echo "    Initial Supply: $INITIAL_SUPPLY"
echo "    Token Recipient: $TOKEN_RECIPIENT"
echo ""
...
if [ ! -f "target/dev/survivor_governance_SurvivorToken.contract_class.json" ]; then
    print_error "SurvivorToken contract build failed or contract file not found"
    print_error "Expected: target/dev/survivor_governance_SurvivorToken.contract_class.json"
    echo "Available contract files:"
    ls -la target/dev/*.contract_class.json 2>/dev/null || echo "No contract files found"
    exit 1
fi

if [ ! -f "target/dev/survivor_governance_SurvivorGovernorController.contract_class.json" ]; then
    print_error "SurvivorGovernorController contract build failed or contract file not found"
    print_error "Expected: target/dev/survivor_governance_SurvivorGovernorController.contract_class.json"
    echo "Available contract files:"
    ls -la target/dev/*.contract_class.json 2>/dev/null || echo "No contract files found"
    exit 1
fi

if [ ! -f "target/dev/survivor_governance_SurvivorGovernor.contract_class.json" ]; then
    print_error "SurvivorGovernor contract build failed or contract file not found"
    print_error "Expected: target/dev/survivor_governance_SurvivorGovernor.contract_class.json"
    echo "Available contract files:"
    ls -la target/dev/*.contract_class.json 2>/dev/null || echo "No contract files found"
    exit 1
fi
...
# ============================
# DECLARE AND DEPLOY VOTING TOKEN
# ============================

print_info "Declaring SurvivorToken contract..."

# Build declare command based on deployment type
if [ "$DEPLOY_TO_SLOT" = "true" ]; then
    TOKEN_DECLARE_OUTPUT=$(starkli declare --account $STARKNET_ACCOUNT --rpc $STARKNET_RPC --watch target/dev/survivor_governance_SurvivorToken.contract_class.json 2>&1)
else
    TOKEN_DECLARE_OUTPUT=$(starkli declare --account $STARKNET_ACCOUNT --rpc $STARKNET_RPC --watch target/dev/survivor_governance_SurvivorToken.contract_class.json --private-key $STARKNET_PK 2>&1)
fi

# Extract class hash from output
TOKEN_CLASS_HASH=$(echo "$TOKEN_DECLARE_OUTPUT" | grep -oE '0x[0-9a-fA-F]+' | tail -1)

if [ -z "$TOKEN_CLASS_HASH" ]; then
    # Contract might already be declared, try to extract from error message
    if echo "$TOKEN_DECLARE_OUTPUT" | grep -q "already declared"; then
        TOKEN_CLASS_HASH=$(echo "$TOKEN_DECLARE_OUTPUT" | grep -oE 'class_hash: 0x[0-9a-fA-F]+' | grep -oE '0x[0-9a-fA-F]+')
        print_warning "SurvivorToken contract already declared with class hash: $TOKEN_CLASS_HASH"
    else
        print_error "Failed to declare SurvivorToken contract"
        echo "$TOKEN_DECLARE_OUTPUT"
        exit 1
    fi
else
    print_info "SurvivorToken contract declared with class hash: $TOKEN_CLASS_HASH"
fi

# Deploy SurvivorToken contract
print_info "Deploying SurvivorToken contract..."

# SurvivorToken constructor parameters:
...
print_info "Token initial supply: $INITIAL_SUPPLY"
print_info "Token recipient: $TOKEN_RECIPIENT"

if [ "$DEPLOY_TO_SLOT" = "true" ]; then
    VOTING_TOKEN_ADDRESS=$(starkli deploy \
...
if [ -z "$VOTING_TOKEN_ADDRESS" ]; then
    print_error "Failed to deploy SurvivorToken contract"
    exit 1
fi

print_info "SurvivorToken contract deployed at address: $VOTING_TOKEN_ADDRESS"
...
# Build declare command based on deployment type
if [ "$DEPLOY_TO_SLOT" = "true" ]; then
    CONTROLLER_DECLARE_OUTPUT=$(starkli declare --account $STARKNET_ACCOUNT --rpc $STARKNET_RPC --watch target/dev/survivor_governance_SurvivorGovernorController.contract_class.json 2>&1)
else
    CONTROLLER_DECLARE_OUTPUT=$(starkli declare --account $STARKNET_ACCOUNT --rpc $STARKNET_RPC --watch target/dev/survivor_governance_SurvivorGovernorController.contract_class.json --private-key $STARKNET_PK 2>&1)
fi
...
# Build declare command based on deployment type
if [ "$DEPLOY_TO_SLOT" = "true" ]; then
    GOV_DECLARE_OUTPUT=$(starkli declare --account $STARKNET_ACCOUNT --rpc $STARKNET_RPC --watch target/dev/survivor_governance_SurvivorGovernor.contract_class.json 2>&1)
else
    GOV_DECLARE_OUTPUT=$(starkli declare --account $STARKNET_ACCOUNT --rpc $STARKNET_RPC --watch target/dev/survivor_governance_SurvivorGovernor.contract_class.json --private-key $STARKNET_PK 2>&1)
fi
...
echo
echo "SurvivorToken Contract:"
echo "  Address: $VOTING_TOKEN_ADDRESS"
echo "  Class Hash: $TOKEN_CLASS_HASH"
echo "  Token Name: $TOKEN_NAME"
echo "  Token Symbol: $TOKEN_SYMBOL"
echo "  Initial Supply: $INITIAL_SUPPLY"
echo "  Recipient: $TOKEN_RECIPIENT"
echo
